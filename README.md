...
Phone identities must use E.164 (`+` plus country code and digits); iMessage email identities must be lowercase. Never put one participant's identity in messages or user-facing output. The listing must belong to the linked seller. Set `PHOTON_IMESSAGE_LINE` on the Photon worker to the dedicated Dibs line: Spectrum group creation does not support shared-line mode. The worker uses this line for new and persisted direct spaces, private relays, marketplace groups, and inbound filtering. The connection tool creates one durable group per buyer/listing pair and reuses it on subsequent calls. Private buyer/seller relay remains the fallback for conversations that have not been connected to a group.
...
Add `PHOTON_PROJECT_ID`, `PHOTON_PROJECT_SECRET`, `PHOTON_IMESSAGE_LINE`, and the existing Supabase variables to `.env.local`, then run `npm run photon:local`. `PHOTON_IMESSAGE_LINE` is required in E.164 format and selects the same dedicated Dibs line for receiving and sending. In production these Photon variables belong on the persistent Railway worker only; the Vercel web application accepts onboarding requests into Supabase and does not read Photon configuration. The worker searches the same real listings as `/api/search`. `PHOTON_DEFAULT_CITY` defaults to `Miami, FL` for the Alpha test.
